...
\graphicspath{{D:/BIALPROJECT/patients/figure_results/}{C:/workspace/figures/}{C:/workspace/figures/}}
...
\affil[1]{The Hospital for Sick Children, Neurosciences and Mental Health program, University of Toronto, Bay St. 686, Toronto, Ontario (Canada)}
\affil[2]{}
\affil[3]{}
%\twocolumn[
...
%Next we perform a multivariate approach to investigate whether the "alpha desynchronization" hypothesis. 
Next, we formulate and calculate the wiring cost of functional connectivity networks. Showing that, on average, it decreases from eyes closed to eyes open.
...
Second, we calculate the functional connectivity for each condition, eyes closed and eyes open, using the connectivity matrices described above. Two Spearman correlation of power time -series and two phase-based connectivity criteria -phase difference synchronization (ISPC) and phase lag index (PLI). %YS figure
...
We find that both the local wiring cost and the mesoscopic wiring cost defined here, decreases on average when going from eyes closed to eyes open.
%YS Supporting...
